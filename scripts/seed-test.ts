/**
 * Test seed — runs AFTER scripts/seed.ts to add E2E test infrastructure.
 *
 * Usage:
 ...
 *  - user1–user10 test fixture users (if not already created by seed.ts)
...
const adminUser = db.select().from(schema.users).all().find(
  (u) => u.role === "admin" && u.name === "Admin",
);
const hostUser = db.select().from(schema.users).all().find((u) => u.name === "Host");

if (!adminUser || !hostUser) {
  throw new Error("seed-test.ts requires seed.ts to have run first (Admin and Host users not found)");
}
...
for (const qaUser of qaCheckoutUserDefs) {
  let login = db.select().from(schema.logins).all().find((l) => l.email === qaUser.email);
  if (!login) {
    const loginId = uuid();
    db.insert(schema.logins)
      .values({ id: loginId, email: qaUser.email, expiresAt: "2099-01-01T00:00:00.000Z" })
      .run();
    login = db.select().from(schema.logins).all().find((l) => l.id === loginId)!;
  }

  let user = db.select().from(schema.users).all().find((u) => u.loginId === login!.id);
  if (!user) {
    const id = uuid();
    db.insert(schema.users)
      ...
      .run();
    user = db.select().from(schema.users).all().find((u) => u.id === id)!;
  }
  ...
  if (!consent.lastProfileUpdate || !consent.locationVerification) {
    db.update(schema.users)
      ...
      .run();
    user = db.select().from(schema.users).all().find((u) => u.id === user!.id)!;
  }
...
for (const e of qaEventDefs) {
  const existing = db.select().from(schema.events).all().find((ev) => ev.title === e.title);
...
function seedInventory(opts: {
  ...
  const existingInv = db.select().from(schema.inventoryGroups).all()
...
seedInventory({
  ...
  salesEndOffset: 25,
  products: [
    { name: "QA Standard Pass",     price: 19, maxQuantity: 6, soldQuantity: 1, participantCapacity: 1 },
    { name: "QA One-Time VIP Seat", price: 49, maxQuantity: 1, soldQuantity: 0, participantCapacity: 1 },
    { name: "QA Premium Pass",      price: 34, maxQuantity: 4, soldQuantity: 0, participantCapacity: 1 },
  ],
});
...
seedInventory({
  ...
  salesEndOffset: 25,
  products: [
    { name: "QA Pair Workshop (2 participants)", price: 54, maxQuantity: 6, soldQuantity: 1, participantCapacity: 2 },
    { name: "QA Trio Lab (3 participants)",       price: 72, maxQuantity: 4, soldQuantity: 0, participantCapacity: 3 },
  ],
});
...
function seedTransaction(opts: {
  ...
  const txId = uuid();
  let totalAmount = 0;
  const itemValues: Array<{ id: string; transactionId: string; productId: string; quantity: number; unitPrice: number }> = [];
  const ticketValues: Array<{ id: string; qrCodeUuid: string; transactionId: string; productId: string; eventId: string; buyerId: string; scannedAt: string | null; createdAt: string }> = [];

  for (const item of opts.items) {
    const product = eventProducts.find((p) => p.name === item.productName);
    if (!product) { console.warn(`  skipping product '${item.productName}' for '${opts.eventTitle}'`); continue; }
    totalAmount += product.price * item.quantity;
    itemValues.push({ id: uuid(), transactionId: txId, productId: product.id, quantity: item.quantity, unitPrice: product.price });
  ...
  for (const iv of itemValues) db.insert(schema.transactionItems).values(iv).run();
  ...
  for (const tv of ticketValues) {
    ...
    for (let p = 1; p <= capacity; p++) {
      const first = p === 1 ? buyer.name : FIRST_NAMES[Math.floor(Math.random() * FIRST_NAMES.length)];
      const last  = p === 1 ? ((buyer as any).familyName ?? "Seed") : LAST_NAMES[Math.floor(Math.random() * LAST_NAMES.length)];
...
seedTransaction({ eventTitle: "North Chapter – Summer Social", buyerIdx: 0, items: [{ productName: "Free Entry", quantity: 1 }], daysAgo: 5 });
seedTransaction({ eventTitle: "North Chapter – Summer Social", buyerIdx: 1, items: [{ productName: "Free Entry", quantity: 2 }], daysAgo: 4 });
seedTransaction({ eventTitle: "North Chapter – Summer Social", buyerIdx: 3, items: [{ productName: "Free Entry", quantity: 1 }], daysAgo: 3 });
seedTransaction({ eventTitle: "North Chapter – Summer Social", buyerIdx: 5, items: [{ productName: "Free Entry", quantity: 1 }], daysAgo: 2 });
seedTransaction({ eventTitle: "North Chapter – Summer Social", buyerIdx: 7, items: [{ productName: "Free Entry", quantity: 1 }], daysAgo: 2 });
seedTransaction({ eventTitle: "North Chapter – Summer Social", buyerIdx: 9, items: [{ productName: "Free Entry", quantity: 2 }], daysAgo: 1 });
seedTransaction({ eventTitle: "North Chapter – Summer Social", buyerIdx: -1, items: [{ productName: "Free Entry", quantity: 1 }], daysAgo: 2, assignedToEmail: "user1@example.com" });

// ── Tech Talk — standard + VIP ──
seedTransaction({ eventTitle: "Tech Talk: AI & Automation", buyerIdx: 0, items: [{ productName: "Standard Seat", quantity: 1 }], daysAgo: 10 });
seedTransaction({ eventTitle: "Tech Talk: AI & Automation", buyerIdx: 1, items: [{ productName: "Standard Seat", quantity: 2 }], daysAgo: 8 });
seedTransaction({ eventTitle: "Tech Talk: AI & Automation", buyerIdx: 5, items: [{ productName: "VIP Seat (front row + networking)", quantity: 1 }], daysAgo: 7 });
seedTransaction({ eventTitle: "Tech Talk: AI & Automation", buyerIdx: 7, items: [{ productName: "Standard Seat", quantity: 1 }], daysAgo: 6 });
seedTransaction({ eventTitle: "Tech Talk: AI & Automation", buyerIdx: -1, items: [{ productName: "VIP Seat (front row + networking)", quantity: 1 }], daysAgo: 12 });
seedTransaction({ eventTitle: "Tech Talk: AI & Automation", buyerIdx: 3, items: [{ productName: "Standard Seat", quantity: 2 }], daysAgo: 5 });
seedTransaction({ eventTitle: "Tech Talk: AI & Automation", buyerIdx: -1, items: [{ productName: "Standard Seat", quantity: 1 }], daysAgo: 3, assignedToEmail: "user2@example.com" });

// ── Welcome Night ──
seedTransaction({ eventTitle: "New Members Welcome Night", buyerIdx: 3, items: [{ productName: "Welcome Night Ticket", quantity: 1 }], daysAgo: 2 });
seedTransaction({ eventTitle: "New Members Welcome Night", buyerIdx: 7, items: [{ productName: "Welcome Night Ticket", quantity: 1 }], daysAgo: 2 });
seedTransaction({ eventTitle: "New Members Welcome Night", buyerIdx: 9, items: [{ productName: "Welcome Night Ticket", quantity: 1 }], daysAgo: 1 });
seedTransaction({ eventTitle: "New Members Welcome Night", buyerIdx: -2, items: [{ productName: "Welcome Night Ticket", quantity: 1 }], daysAgo: 3 });
seedTransaction({ eventTitle: "New Members Welcome Night", buyerIdx: -1, items: [{ productName: "Welcome Night Ticket", quantity: 1 }], daysAgo: 1, assignedToEmail: "user3@example.com" });

// ── Community Hike ──
seedTransaction({ eventTitle: "South Chapter – Community Hike", buyerIdx: 2, items: [{ productName: "Hiker Spot", quantity: 1 }], daysAgo: 6 });
seedTransaction({ eventTitle: "South Chapter – Community Hike", buyerIdx: 5, items: [{ productName: "Hiker Spot", quantity: 2 }], daysAgo: 4 });
seedTransaction({ eventTitle: "South Chapter – Community Hike", buyerIdx: 7, items: [{ productName: "Hiker Spot", quantity: 1 }], daysAgo: 3 });
seedTransaction({ eventTitle: "South Chapter – Community Hike", buyerIdx: -1, items: [{ productName: "Hiker Spot", quantity: 1 }], daysAgo: 5 });

// ── Workshop (paid) ──
seedTransaction({ eventTitle: "Workshop: Public Speaking Skills", buyerIdx: 2, items: [{ productName: "Workshop Ticket", quantity: 1 }], daysAgo: 14 });
seedTransaction({ eventTitle: "Workshop: Public Speaking Skills", buyerIdx: 5, items: [{ productName: "Workshop Ticket", quantity: 1 }], daysAgo: 12 });
seedTransaction({ eventTitle: "Workshop: Public Speaking Skills", buyerIdx: 7, items: [{ productName: "Workshop Ticket", quantity: 2 }], daysAgo: 10 });
seedTransaction({ eventTitle: "Workshop: Public Speaking Skills", buyerIdx: 0, items: [{ productName: "Workshop Ticket", quantity: 1 }], daysAgo: 9 });
seedTransaction({ eventTitle: "Workshop: Public Speaking Skills", buyerIdx: -1, items: [{ productName: "Workshop Ticket", quantity: 2 }], daysAgo: 15 });

// ── Morning Walk ──
seedTransaction({ eventTitle: "Coast Chapter – Morning Walk", buyerIdx: 1, items: [{ productName: "Walk Ticket", quantity: 1 }], daysAgo: 3 });
seedTransaction({ eventTitle: "Coast Chapter – Morning Walk", buyerIdx: 6, items: [{ productName: "Walk Ticket", quantity: 1 }], daysAgo: 2 });
seedTransaction({ eventTitle: "Coast Chapter – Morning Walk", buyerIdx: 3, items: [{ productName: "Walk Ticket", quantity: 1 }], daysAgo: 1 });

// ── Open Meetup ──
seedTransaction({ eventTitle: "Coast Chapter – Open Meetup", buyerIdx: 1, items: [{ productName: "Free Entry", quantity: 1 }], daysAgo: 4 });
seedTransaction({ eventTitle: "Coast Chapter – Open Meetup", buyerIdx: 3, items: [{ productName: "Free Entry", quantity: 2 }], daysAgo: 3 });
seedTransaction({ eventTitle: "Coast Chapter – Open Meetup", buyerIdx: 6, items: [{ productName: "Free Entry", quantity: 1 }], daysAgo: 2 });
seedTransaction({ eventTitle: "Coast Chapter – Open Meetup", buyerIdx: 8, items: [{ productName: "Free Entry", quantity: 2 }], daysAgo: 1 });

// ── Networking Breakfast (paid) ──
seedTransaction({ eventTitle: "Networking Breakfast", buyerIdx: 2, items: [{ productName: "Breakfast Ticket (incl. food)", quantity: 1 }], daysAgo: 7 });
seedTransaction({ eventTitle: "Networking Breakfast", buyerIdx: 4, items: [{ productName: "Breakfast Ticket (incl. food)", quantity: 2 }], daysAgo: 5 });
seedTransaction({ eventTitle: "Networking Breakfast", buyerIdx: 8, items: [{ productName: "Breakfast Ticket (incl. food)", quantity: 1 }], daysAgo: 4 });
seedTransaction({ eventTitle: "Networking Breakfast", buyerIdx: -2, items: [{ productName: "Breakfast Ticket (incl. food)", quantity: 1 }], daysAgo: 6 });

// ── Startup Pitch Night (paid + free pitcher) ──
seedTransaction({ eventTitle: "Startup Pitch Night", buyerIdx: 4, items: [{ productName: "Audience Ticket", quantity: 2 }], daysAgo: 10 });
seedTransaction({ eventTitle: "Startup Pitch Night", buyerIdx: 8, items: [{ productName: "Audience Ticket", quantity: 3 }], daysAgo: 8 });
seedTransaction({ eventTitle: "Startup Pitch Night", buyerIdx: 2, items: [{ productName: "Audience Ticket", quantity: 2 }], daysAgo: 7 });
seedTransaction({ eventTitle: "Startup Pitch Night", buyerIdx: 6, items: [{ productName: "Audience Ticket", quantity: 1 }], daysAgo: 6 });
seedTransaction({ eventTitle: "Startup Pitch Night", buyerIdx: 0, items: [{ productName: "Pitcher Ticket (present your startup)", quantity: 1 }], daysAgo: 14 });
seedTransaction({ eventTitle: "Startup Pitch Night", buyerIdx: 4, items: [{ productName: "Pitcher Ticket (present your startup)", quantity: 1 }], daysAgo: 12 });
seedTransaction({ eventTitle: "Startup Pitch Night", buyerIdx: -1, items: [{ productName: "Audience Ticket", quantity: 4 }], daysAgo: 9 });
seedTransaction({ eventTitle: "Startup Pitch Night", buyerIdx: 9, items: [{ productName: "Pitcher Ticket (present your startup)", quantity: 1 }], daysAgo: 11 });

// ── Members Dinner (paid) ──
seedTransaction({ eventTitle: "West Chapter Members Dinner", buyerIdx: 3, items: [{ productName: "Dinner Ticket (3-course meal)", quantity: 2 }], daysAgo: 5 });
seedTransaction({ eventTitle: "West Chapter Members Dinner", buyerIdx: 8, items: [{ productName: "Dinner Ticket (3-course meal)", quantity: 1 }], daysAgo: 4 });
seedTransaction({ eventTitle: "West Chapter Members Dinner", buyerIdx: -2, items: [{ productName: "Dinner Ticket (3-course meal)", quantity: 2 }], daysAgo: 6 });
seedTransaction({ eventTitle: "West Chapter Members Dinner", buyerIdx: 4, items: [{ productName: "Dinner Ticket (3-course meal)", quantity: 1 }], daysAgo: 3 });
seedTransaction({ eventTitle: "West Chapter Members Dinner", buyerIdx: 0, items: [{ productName: "Dinner Ticket (3-course meal)", quantity: 2 }], daysAgo: 7 });

// ── Meet & Greet ──
seedTransaction({ eventTitle: "West Chapter – Public Meet & Greet", buyerIdx: 3, items: [{ productName: "Free Entry", quantity: 1 }], daysAgo: 3 });
seedTransaction({ eventTitle: "West Chapter – Public Meet & Greet", buyerIdx: 8, items: [{ productName: "Free Entry", quantity: 1 }], daysAgo: 2 });
seedTransaction({ eventTitle: "West Chapter – Public Meet & Greet", buyerIdx: -2, items: [{ productName: "Free Entry", quantity: 1 }], daysAgo: 2 });
seedTransaction({ eventTitle: "West Chapter – Public Meet & Greet", buyerIdx: 4, items: [{ productName: "Free Entry", quantity: 1 }], daysAgo: 1 });
...
console.log(`  transactions seeded (${txCount} transactions, ${ticketCount} tickets)`);
...
console.log("    Search users: Marta Silva, Jonas Berg, Leonie Dubois, Tariq Hassan");
...
const electionCycleDefs: Array<{
  ...
  { title: "Board Elections 2024", year: 2024, description: "Annual board elections for the 2024–2026 term.", status: "closed", requiredMembershipTier: "full" },
  { title: "Board Elections 2026", year: 2026, description: "Nominations are open for the 2026–2028 board term. Full members may apply.", status: "open", requiredMembershipTier: "full" },
];
...
const applicationDefs: Array<{
  cycleTitle: string; positionTitle: string; userIdx: number;
...
for (const a of applicationDefs) {
  ...
  db.insert(schema.electionApplications)
    .values({
      id: uuid(), positionId, cycleId, userId, status: a.status,
...
console.log(`  elections seeded (${cycleCount} cycles, ${posCount} positions, ${appCount} applications)`);
