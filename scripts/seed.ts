...
async function downloadAndUploadSeedImages(): Promise<string[]> {
	...
	for (let i = 0; i < seedImageSources.length; i++) {
		...
		if (!response.ok) {
			throw new Error(
				`Failed to download seed image: ${sourceUrl} (${response.status})`,
			);
		}
...
const knownSpecialEmails = new Set([ADMIN_EMAIL, "host@example.com"]);
...
// Assign bulk faker users to groups (1–2 groups each, random)
{
	...
	console.log(
		`  bulk user memberships seeded (${bulkMembershipCount} across ${bulkUserIds.length} users)`,
	);
}
...
// Add faker-generated posts for each group (3 per group)
{
	...
	const fakerPostTopics = [
		...
		() => ({
			title: `Upcoming: ${faker.lorem.words(3)}`,
			body: `<p>${faker.lorem.paragraph()}</p><p>${faker.lorem.paragraph()}</p>`,
		}),
		() => ({
			title: `Member spotlight: ${faker.person.firstName()} ${faker.person.lastName()}`,
			body: `<p>${faker.lorem.paragraph()}</p><p>${faker.lorem.paragraph()}</p>`,
		}),
		() => ({
			title: `Community update — ${faker.date.month()}`,
			body: `<p>${faker.lorem.paragraph()}</p>`,
		}),
		() => ({
			title: faker.lorem.sentence({ min: 4, max: 7 }).replace(/\.$/, ""),
			body: `<p>${faker.lorem.paragraph()}</p><p>${faker.lorem.paragraph()}</p>`,
		}),
	];

	for (const [, groupId] of groupSlugsForPosts) {
		const existingCount = allGroupPosts.filter(
			(p) => p.groupId === groupId,
		).length;
		...
		for (let i = 0; i < toAdd; i++) {
			...
			const createdAt = faker.date
				.between({ from: "2023-01-01", to: new Date() })
				.toISOString();
			db.insert(schema.posts)
				.values({
					id: uuid(),
...
for (const e of eventDefs) {
	...
	db.insert(schema.events)
		.values({
			...
			latitude: e.latitude,
			onlineUrl:
				e.city === "Online" ? "https://meet.example.com/town-hall" : null,
...
for (const eventTitle of statusEvents) {
	...
	for (let i = 0; i < usersForEvent.length; i++) {
		...
		const existing = db
			...
			.all()
			.find((p) => p.userId === usersForEvent[i].id && p.eventId === evt.id);
...
seedInventory({
	...
	products: [
		...
		{
			name: "VIP Entry (incl. welcome drink)",
			price: 30,
			maxQuantity: 20,
			soldQuantity: 5,
		},
...
seedInventory({
	...
	products: [
		...
		{
			name: "VIP Seat (front row + networking)",
			price: 15,
			maxQuantity: 5,
			soldQuantity: 2,
		},
...
seedInventory({
	...
	products: [
		{
			name: "Breakfast Ticket (incl. food)",
			price: 12,
			maxQuantity: 20,
			soldQuantity: 5,
		},
...
seedInventory({
	...
	products: [
		...
		{
			name: "Pitcher Ticket (present your startup)",
			price: 0,
			maxQuantity: 5,
			soldQuantity: 3,
		},
...
seedInventory({
	...
	products: [
		{
			name: "Dinner Ticket (3-course meal)",
			price: 25,
			maxQuantity: 20,
			soldQuantity: 8,
		},
...
seedInventory({
	...
	products: [
		{
			name: "AI Hands-on Workshop",
			price: 22,
			maxQuantity: 30,
			soldQuantity: 4,
		},
		{
			name: "Founder Coaching Session",
			price: 45,
			maxQuantity: 20,
			soldQuantity: 3,
		},
...

{
	...
	function seedHistoricalEvent(opts: {
		...
		if (
			db
				...
				.from(schema.events)
				.all()
				.find((e) => e.title === opts.title)
		)
			return;

		const eventId = uuid();
		const endDate = new Date(
			new Date(opts.startDate).getTime() + opts.durationHours * 3_600_000,
		).toISOString();
		const createdAt = new Date(
			new Date(opts.startDate).getTime() - 30 * 86_400_000,
		).toISOString();
		...
		db.insert(schema.inventoryGroups)
			.values({
				...
				salesStartDate: new Date(
					new Date(opts.startDate).getTime() - 60 * 86_400_000,
				).toISOString(),
				salesEndDate: new Date(
					new Date(opts.startDate).getTime() - 86_400_000,
				).toISOString(),
				createdAt,
			})
			.run();
		...
		for (const product of opts.products) {
			...
			db.insert(schema.products)
				.values({
					...
					name: product.name,
					price: product.price,
					maxQuantity: product.capacity,
					soldQuantity: product.attendees,
			...
			for (const buyerId of pickBuyers(product.attendees)) {
				...
				const buyerEmail = buyer.loginId
					? (loginEmailById.get(buyer.loginId) ?? null)
					: null;
				...
				db.insert(schema.transactions)
					.values({
						...
						transactionFee:
							product.price > 0
								? Math.round(product.price * 0.029 * 100) / 100
								: 0,
						stripeSessionId: `cs_hist_${uuid().slice(0, 8)}`,
						stripePaymentId:
							product.price > 0 ? `pi_hist_${uuid().slice(0, 8)}` : null,
						paymentDate: purchaseDate,
						createdAt: purchaseDate,
					})
					.run();
				db.insert(schema.transactionItems)
					.values({
						id: uuid(),
						transactionId: txId,
						productId,
				...
				db.insert(schema.tickets)
					.values({
						...
						buyerId,
						scannedAt: new Date(
							new Date(opts.startDate).getTime() + Math.random() * 3_600_000,
						).toISOString(),
						createdAt: purchaseDate,
					})
					.run();
	...
	// ── Global events ─────────────────────────────────────────────────────────
	seedHistoricalEvent({
		title: "Community Founding Meetup",
		body: p(faker.lorem.paragraph()),
		groupSlug: null,
		startDate: "2015-06-12T18:00:00.000Z",
		durationHours: 3,
		city: "Demo City",
		country: "Exampleland",
		address: "The Foundry, Demo City",
		latitude: "52.5200",
		longitude: "13.4050",
		locationType: "in-person",
		products: [{ name: "Free Entry", price: 0, capacity: 60, attendees: 42 }],
	});
	seedHistoricalEvent({
		title: "Annual Gathering 2016",
		body: p(faker.lorem.paragraph()),
		groupSlug: null,
		startDate: "2016-10-08T10:00:00.000Z",
		durationHours: 8,
		city: "Demo City",
		country: "Exampleland",
		address: "Grand Hall, Demo City",
		latitude: "52.5200",
		longitude: "13.4050",
		locationType: "in-person",
		products: [
			{ name: "Standard Ticket", price: 15, capacity: 80, attendees: 68 },
			{ name: "VIP Ticket", price: 35, capacity: 20, attendees: 14 },
		],
	});
	seedHistoricalEvent({
		title: "Summer Social 2017",
		body: p(faker.lorem.paragraph()),
		groupSlug: null,
		startDate: "2017-07-14T17:00:00.000Z",
		durationHours: 4,
		city: "Demo City",
		country: "Exampleland",
		address: "Riverside Park, Demo City",
		latitude: "52.5200",
		longitude: "13.4050",
		locationType: "in-person",
		products: [{ name: "Free Entry", price: 0, capacity: 80, attendees: 63 }],
	});
	seedHistoricalEvent({
		title: "Annual Conference 2018",
		body: p(faker.lorem.paragraph()),
		groupSlug: null,
		startDate: "2018-09-21T09:00:00.000Z",
		durationHours: 9,
		city: "Demo City",
		country: "Exampleland",
		address: "Convention Centre, Demo City",
		...
		locationType: "in-person",
		products: [
			{ name: "Day Pass", price: 20, capacity: 120, attendees: 98 },
			{ name: "VIP Pass", price: 45, capacity: 30, attendees: 22 },
		],
	});
	seedHistoricalEvent({
		title: "Annual Conference 2019",
		body: p(faker.lorem.paragraph()),
		groupSlug: null,
		startDate: "2019-10-05T09:00:00.000Z",
		durationHours: 10,
		city: "Southville",
		country: "Exampleland",
		address: "Expo Centre, Southville",
		latitude: "41.3851",
		longitude: "2.1734",
		locationType: "in-person",
		products: [
			{ name: "Standard Pass", price: 25, capacity: 160, attendees: 143 },
			{ name: "Premium Pass", price: 55, capacity: 40, attendees: 31 },
		],
	});
	seedHistoricalEvent({
		title: "Online Town Hall — Spring 2020",
		body: p(
			"Our first fully online event, bringing all chapters together virtually.",
		),
		groupSlug: null,
		startDate: "2020-05-20T17:00:00.000Z",
		durationHours: 2,
		city: "Online",
		country: "Exampleland",
		address: "",
		...
		longitude: "13.4050",
		locationType: "online",
		onlineUrl: "https://meet.example.com/townhall-2020",
		products: [{ name: "Free Entry", price: 0, capacity: 200, attendees: 147 }],
	});
	seedHistoricalEvent({
		title: "Online Workshop — Autumn 2020",
		body: p(faker.lorem.paragraph()),
		groupSlug: null,
		startDate: "2020-11-14T10:00:00.000Z",
		durationHours: 3,
		city: "Online",
		country: "Exampleland",
		address: "",
		...
		longitude: "13.4050",
		locationType: "online",
		onlineUrl: "https://meet.example.com/workshop-2020",
		products: [{ name: "Free Entry", price: 0, capacity: 150, attendees: 89 }],
	});
	seedHistoricalEvent({
		title: "First In-Person Since 2020",
		body: p(
			"A long-awaited return to meeting face-to-face. " +
				faker.lorem.sentence(),
		),
		groupSlug: null,
		startDate: "2021-07-03T15:00:00.000Z",
		durationHours: 4,
		city: "Demo City",
		country: "Exampleland",
		address: "Rooftop Bar, Demo City",
		latitude: "52.5200",
		longitude: "13.4050",
		locationType: "in-person",
		products: [{ name: "Free Entry", price: 0, capacity: 80, attendees: 71 }],
	});
	seedHistoricalEvent({
		title: "Annual Conference 2022",
		body: p(faker.lorem.paragraph()),
		groupSlug: null,
		...
		city: "Portstown",
		country: "Exampleland",
		address: "Harbour Convention Centre, Portstown",
		latitude: "51.5074",
		longitude: "-0.1278",
		locationType: "in-person",
		products: [
			{ name: "Standard Pass", price: 30, capacity: 200, attendees: 176 },
			{ name: "VIP Pass", price: 60, capacity: 50, attendees: 38 },
		],
	});
	seedHistoricalEvent({
		...
		body: p(faker.lorem.paragraph()),
		groupSlug: null,
		startDate: "2023-07-22T16:00:00.000Z",
		durationHours: 5,
		city: "Demo City",
		country: "Exampleland",
		address: "Rooftop Terrace, Demo City",
		latitude: "52.5200",
		longitude: "13.4050",
		locationType: "in-person",
		products: [{ name: "Free Entry", price: 0, capacity: 100, attendees: 83 }],
	});
	seedHistoricalEvent({
		title: "Annual Conference 2024",
		body: p(faker.lorem.paragraph()),
		groupSlug: null,
		startDate: "2024-04-19T09:00:00.000Z",
		durationHours: 10,
		city: "Midtown",
		country: "Exampleland",
		address: "Innovation Hub, Midtown",
		latitude: "48.2082",
		longitude: "16.3738",
		locationType: "in-person",
		products: [
			{ name: "Standard Pass", price: 35, capacity: 250, attendees: 214 },
			{ name: "VIP Pass", price: 70, capacity: 50, attendees: 41 },
		],
	});

	// ── Local chapter events ──────────────────────────────────────────────────
	seedHistoricalEvent({
		title: "North Chapter — Spring Social 2016",
		body: p(faker.lorem.paragraph()),
		groupSlug: "berlin",
		startDate: "2016-04-09T14:00:00.000Z",
		durationHours: 3,
		city: "Demo City",
		country: "Exampleland",
		address: "Local Café, Demo City",
		latitude: "59.3293",
		longitude: "18.0686",
		locationType: "in-person",
		products: [{ name: "Free Entry", price: 0, capacity: 30, attendees: 23 }],
	});
	seedHistoricalEvent({
		title: "South Chapter — Hiking Day 2018",
		body: p(faker.lorem.paragraph()),
		groupSlug: "munich",
		startDate: "2018-05-19T09:00:00.000Z",
		durationHours: 5,
		city: "Southville",
		country: "Exampleland",
		address: "Trailhead, Southville",
		latitude: "41.3851",
		longitude: "2.1734",
		locationType: "in-person",
		products: [{ name: "Free Entry", price: 0, capacity: 25, attendees: 19 }],
	});
	seedHistoricalEvent({
		title: "Coast Chapter — Harbour Walk 2019",
		body: p(faker.lorem.paragraph()),
		groupSlug: "hamburg",
		startDate: "2019-09-07T10:00:00.000Z",
		durationHours: 2,
		city: "Portstown",
		country: "Exampleland",
		address: "Harbourside, Portstown",
		...
		longitude: "-0.1278",
		locationType: "in-person",
		products: [{ name: "Free Entry", price: 0, capacity: 20, attendees: 17 }],
	});
	seedHistoricalEvent({
		title: "Central Chapter — Networking Dinner 2021",
		body: p(faker.lorem.paragraph()),
		groupSlug: "frankfurt",
		startDate: "2021-10-14T19:00:00.000Z",
		durationHours: 3,
		city: "Midtown",
		country: "Exampleland",
		address: "The Grand Café, Midtown",
		...
		locationType: "in-person",
		products: [
			{ name: "Dinner Ticket", price: 18, capacity: 40, attendees: 32 },
		],
	});
	seedHistoricalEvent({
		title: "West Chapter — End of Year Party 2022",
		body: p(faker.lorem.paragraph()),
		groupSlug: "cologne",
		startDate: "2022-12-02T19:00:00.000Z",
		durationHours: 4,
		city: "Westford",
		country: "Exampleland",
		address: "Town Square Venue, Westford",
		latitude: "48.8566",
		longitude: "2.3522",
		locationType: "in-person",
		products: [
			{ name: "Entry Ticket", price: 10, capacity: 50, attendees: 41 },
		],
	});
	seedHistoricalEvent({
		title: "North Chapter — Annual Dinner 2023",
		body: p(faker.lorem.paragraph()),
		groupSlug: "berlin",
		...
		city: "Demo City",
		country: "Exampleland",
		address: "The Riverside Restaurant, Demo City",
		latitude: "59.3293",
		longitude: "18.0686",
		locationType: "in-person",
		products: [
			{ name: "Dinner Ticket", price: 22, capacity: 45, attendees: 38 },
		],
	});
	seedHistoricalEvent({
		title: "South Chapter — Workshop Day 2024",
		body: p(faker.lorem.paragraph()),
		groupSlug: "munich",
		startDate: "2024-02-24T10:00:00.000Z",
		durationHours: 6,
		city: "Southville",
		country: "Exampleland",
		address: "Community Centre, Southville",
		...
		locationType: "in-person",
		products: [
			{ name: "Workshop Ticket", price: 12, capacity: 30, attendees: 27 },
		],
	});
	...
	const pastCount = db
		...
		.from(schema.events)
		.all()
		.filter((e) => new Date(e.startDate) < new Date()).length;
	console.log(
		`  historical events seeded (${pastCount} total past events with participants)`,
	);
}
...
const eventsNoImg = sqlite
	.prepare(
		"SELECT id FROM events WHERE deleted_at IS NULL AND (image1 IS NULL OR image1 LIKE '%picsum.photos%' OR image2 IS NULL OR image2 LIKE '%picsum.photos%')",
	)
	.all() as Array<{ id: string }>;
const updateEvent = sqlite.prepare(
...
const postsNoImg = sqlite
	.prepare(
		"SELECT id FROM posts WHERE deleted_at IS NULL AND (featured_image IS NULL OR featured_image LIKE '%picsum.photos%')",
	)
	.all() as Array<{ id: string }>;
const updatePost = sqlite.prepare(
...
const groupsNoImg = sqlite
	.prepare(
		"SELECT id FROM groups WHERE (image1 IS NULL OR image1 LIKE '%picsum.photos%' OR image2 IS NULL OR image2 LIKE '%picsum.photos%' OR image3 IS NULL OR image3 LIKE '%picsum.photos%')",
	)
	.all() as Array<{ id: string }>;
...
console.log(
	`  images downloaded/uploaded and linked (${eventsNoImg.length} events, ${postsNoImg.length} posts, ${groupsNoImg.length} groups)`,
);
console.log(
	`  user profile pictures assigned (${usersWithoutProfile.length} users)`,
);
...
const formDefs: Array<{
	...
	},
	{
		...
		description:
			"Tell us what you think about the community and how we can improve.",
		schemaJson: {
			pages: [
				{
					name: "feedback_page",
...
for (const f of formDefs) {
	const existing = f.systemKey
		? db
				...
				.from(schema.forms)
				.all()
				.find((r) => r.systemKey === f.systemKey)
		: null;

	if (existing) {
		if (
			f.allowResubmission !== undefined &&
			existing.allowResubmission !== f.allowResubmission
		) {
...
if (onboardingForm) {
	...
	const affiliationPool = [
		{
			status: "graduate",
			faculty: "engineering",
			programme_engineering: "software_engineering",
			degree_level: "master",
			graduation_year: 2022,
			how_did_you_hear: "classmate",
		},
		{
			status: "graduate",
			faculty: "science",
			programme_science: "computer_science",
			degree_level: "master",
		...
		},
		{
			...
			programme_social: "economics",
			degree_level: "bachelor",
			graduation_year: 2023,
			how_did_you_hear: "professor",
		},
		{
			status: "graduate",
			faculty: "engineering",
			programme_engineering: "electrical_engineering",
			degree_level: "bachelor",
			graduation_year: 2020,
			how_did_you_hear: "uni_email",
		},
		{
			status: "graduate",
			faculty: "arts",
			programme_arts: "media_studies",
			degree_level: "bachelor",
			graduation_year: 2019,
			how_did_you_hear: "event",
		},
		{
			status: "graduate",
			faculty: "medicine",
			programme_medicine: "medicine",
			degree_level: "master",
		...
		},
		{
			status: "graduate",
			faculty: "science",
			programme_science: "data_science",
			degree_level: "master",
			graduation_year: 2023,
			how_did_you_hear: "social_media",
		},
		...
		},
		{
			status: "graduate",
			faculty: "engineering",
			programme_engineering: "civil_engineering",
			degree_level: "bachelor",
		...
		},
		{
			...
			programme_architecture: "architecture",
			degree_level: "master",
			graduation_year: 2020,
			how_did_you_hear: "classmate",
		},
		...
		},
		{
			status: "current_student",
			faculty: "social_sciences",
			programme_social: "business_admin",
			degree_level: "master",
			current_year: "year_1",
			how_did_you_hear: "social_media",
		},
		...
		},
		{
			status: "current_student",
			faculty: "engineering",
			programme_engineering: "mechanical_engineering",
			degree_level: "master",
			current_year: "postgrad_year",
			how_did_you_hear: "classmate",
		},
		{ status: "staff", faculty: "science", how_did_you_hear: "other" },
		{
			status: "graduate",
			faculty: "social_sciences",
			programme_social: "political_science",
			degree_level: "master",
			graduation_year: 2019,
	...
	for (const user of usersToSeed) {
		...
		const resultJson =
			affiliationPool[Math.floor(Math.random() * affiliationPool.length)];
	...
	console.log(
		`  onboarding form results seeded (${formResultsInserted} inserted)`,
	);
}
...
}> = [
	{
		...
		content: [
			...
			},
			{
				...
				order: 2,
				data: { text: "Upcoming Events", level: "2", align: "center" },
			},
			...
			},
			{
				...
				componentType: "TitleBlock",
				order: 5,
				data: { text: "Our Communities", level: "2", align: "center" },
			},
			...
			},
			{
				...
				componentType: "TitleBlock",
				order: 8,
				data: { text: "Latest Posts", level: "2", align: "center" },
			},
			{
				id: uuid(),
		...
		content: [
			{
				...
				componentType: "TitleBlock",
				order: 0,
				data: { text: "About This Community", level: "1", align: "center" },
			},
			{
				id: uuid(),
				componentType: "SpacerBlock",
			...
			},
			{
				...
				data: {
					content:
						"<p>We started as a small group of people who wanted to build a space to connect beyond the usual networking events. What began as a single local chapter quickly grew into a network of communities across different cities.</p><p>Today, we are a growing community with members spread across multiple chapters. Our mission is simple: bring people together, foster genuine relationships, and create opportunities for personal and professional growth.</p>",
				},
			},
			{
				id: uuid(),
			...
			},
			{
				...
				componentType: "TitleBlock",
				order: 4,
				data: { text: "Our Values", level: "2", align: "left" },
			},
			{
				id: uuid(),
				componentType: "TextBlock",
				...
				data: {
					content:
						"<p><strong>Openness</strong> — Everyone is welcome, regardless of background or experience.</p><p><strong>Authenticity</strong> — We value real connections over superficial networking.</p><p><strong>Local Roots</strong> — Each chapter is shaped by its members and the place it calls home.</p>",
				},
			},
			{
				id: uuid(),
			...
			},
			{
				...
				order: 7,
				data: {
					quote: "Great communities are built on trust, not transactions.",
		...
		content: [
			{
				...
				componentType: "TitleBlock",
				order: 0,
				data: { text: "Block Gallery", level: "1", align: "center" },
			},
			{
				id: uuid(),
				componentType: "TextBlock",
				...
				data: {
					content:
						"<p>This page showcases all available content blocks. Open the page builder in the admin to add, remove, and configure them on any page.</p>",
				},
			},
			{
				id: uuid(),
...

			{
				...
				componentType: "TitleBlock",
				order: 3,
				data: { text: "Callout", level: "2", align: "left" },
			},
			{
				id: uuid(),
				...
				data: {
					type: "info",
					title: "Info callout",
					body: "Use this for helpful tips, notes, or supplementary information.",
				},
			},
			{
				id: uuid(),
				...
				data: {
					type: "tip",
					title: "Tip callout",
					body: "Great for pro tips or recommended actions.",
				},
			},
			{
				id: uuid(),
				...
				data: {
					...
					body: "Draws attention to something the reader should be careful about.",
				},
			},
			{
				id: uuid(),
				...
				data: {
					...
					body: "Reserved for critical alerts or irreversible actions.",
				},
			},
			{
				id: uuid(),
...

			{
				...
				componentType: "TitleBlock",
				order: 9,
				data: { text: "Quote", level: "2", align: "left" },
			},
			{
				id: uuid(),
				componentType: "QuoteBlock",
...

			{
				...
				componentType: "TitleBlock",
				order: 12,
				data: { text: "Accordion", level: "2", align: "left" },
			},
			{
				id: uuid(),
					...
					items: JSON.stringify([
						{
							...
							answer:
								"Browse the Communities page and click 'View Group' on any chapter. From there you can join with a single click.",
						},
						{
							question: "Can I create my own chapter?",
							answer:
								"Yes — contact an admin to request a new chapter for your city or region.",
						},
						{
							question: "How do I post an event?",
							answer:
								"Events are created from the admin panel. Navigate to Admin → Events → New Event.",
						},
						{
							question: "Is membership free?",
							answer:
								"Basic membership is always free. Some events may have a ticket price set by the organiser.",
						},
					]),
				},
			},
...

			{
				...
				componentType: "TitleBlock",
				order: 15,
				data: { text: "Video", level: "2", align: "left" },
			},
			{
				id: uuid(),
				componentType: "TextBlock",
				...
				data: {
					content:
						"<p>Paste any YouTube, Vimeo, or direct <code>.mp4</code> URL into the Video block config to embed it.</p>",
				},
			},
			{
				id: uuid(),
				...
				data: {
					...
					caption: "Example: a YouTube video embedded with the Video block.",
				},
			},
			{
				id: uuid(),
...

			{
				...
				componentType: "TitleBlock",
				order: 19,
				data: { text: "Action Button", level: "2", align: "left" },
			},
			{
				id: uuid(),
...

			{
				...
				componentType: "TitleBlock",
				order: 22,
				data: { text: "Dynamic Blocks", level: "2", align: "left" },
			},
			{
				id: uuid(),
				componentType: "CalloutBlock",
				...
				data: {
					type: "info",
					title: "Self-fetching blocks",
					body: "The blocks below (Upcoming Events, Groups List, Posts List) fetch their own data from the database. Drop them on any page and they update automatically.",
				},
			},
			{
				id: uuid(),
			...
			},
			{
				...
				componentType: "TitleBlock",
				order: 25,
				data: { text: "Upcoming Events", level: "3", align: "left" },
			},
		...
		content: [
			{
				...
				componentType: "TitleBlock",
				order: 0,
				data: { text: "Events", level: "1", align: "center" },
			},
			{
				id: uuid(),
				componentType: "SpacerBlock",
			...
			},
			{
				...
				componentType: "TitleBlock",
				order: 4,
				data: { text: "Past Events", level: "2", align: "center" },
			},
		...
		content: [
			{
				...
				componentType: "TitleBlock",
				order: 0,
				data: { text: "Our Communities", level: "1", align: "center" },
			},
			{
				id: uuid(),
				componentType: "SpacerBlock",
			...
			},
			{
				...
				data: {
					content:
						"<p>Find your local community and connect with members in your area.</p>",
				},
			},
			{
				id: uuid(),
		...
		content: [
			{
				...
				componentType: "TitleBlock",
				order: 0,
				data: { text: "Member Deals", level: "1", align: "center" },
			},
			{
				id: uuid(),
				componentType: "SpacerBlock",
			...
			},
			{
				...
				data: {
					content:
						"<p>Exclusive discounts and offers for community members.</p>",
				},
			},
			{
				id: uuid(),
		...
		content: [
			{
				...
				componentType: "TitleBlock",
				order: 0,
				data: { text: "News", level: "1", align: "center" },
			},
			{
				id: uuid(),
				componentType: "SpacerBlock",
...
const dealDefs: Array<{
	...
	},
	{
		...
		description:
			"Get your first month completely free at any FitSpace gym location.",
	...
	},
	{
		...
		description:
			"Work from one of our co-working partner spaces at a special member rate.",
	...
	},
	{
		...
		description:
			"This deal has already expired — used to test expiry filtering.",
...
for (const d of dealDefs) {
	...
	const validUntil =
		d.validUntilOffsetDays !== null
			? daysFromNow(d.validUntilOffsetDays)
			: null;
...
console.log(
	`  jobs seeded (${jobDefs.length}: ${jobDefs.filter((j) => j.status === "approved").length} approved, ${jobDefs.filter((j) => j.status === "pending").length} pending, 1 expired)`,
);

// ─── 17b. Tag definitions ─────────────────────────────────────────────────────

const tagDefDefs: Array<{
	slug: string;
	label: string;
	...
	// Board
	{
		...
		label: "Board Member",
		category: "board",
		description: "Serves on the organisation's board.",
	},
	{
		slug: "board-president",
		label: "President",
		category: "board",
	...
	},
	{
		...
		label: "Secretary",
		category: "board",
		description: "Secretary of the board.",
	},
	{
		slug: "board-treasurer",
		label: "Treasurer",
		category: "board",
		description: "Treasurer of the board.",
	},
	// Participation
	{
		slug: "event-speaker",
		...
		description: "Has spoken at one of our events.",
	},
	{
		slug: "event-volunteer",
		label: "Event Volunteer",
		category: "participation",
		description: "Has volunteered at one of our events.",
	},
	{
		slug: "program-graduate",
		label: "Program Graduate",
		category: "participation",
		description: "Completed one of our programs.",
	},
	// Custom
	{
		slug: "mentor",
		label: "Mentor",
		category: "custom",
		description: "Active mentor in the community.",
	},
	{
		slug: "ambassador",
		label: "Ambassador",
		category: "custom",
		description: "Community ambassador in their city or region.",
	},
	{
		slug: "chapter-lead",
		label: "Chapter Lead",
		category: "custom",
		description: "Leads a local chapter.",
	},
	{
		slug: "founding-member",
		label: "Founding Member",
		category: "custom",
		description: "Was part of the community from the very beginning.",
	},
];
...
const qualTypeDefs = [
	{
		slug: "master-school",
		label: "Master School Graduate",
		description: "Completed our master-level programme.",
		grantsMembershipTier: "full" as const,
	},
	{
		slug: "summer-school",
		label: "Summer School Graduate",
		description: "Completed our summer school programme.",
		grantsMembershipTier: "associated" as const,
	},
	{
		slug: "phd",
		label: "PhD",
		description: "Holds a doctoral degree.",
		grantsMembershipTier: "full" as const,
	},
	{
		slug: "former-employee",
		label: "Former Employee",
		description: "Previously worked at the organisation.",
		grantsMembershipTier: "associated" as const,
	},
];
	...
	if (existing) {
		...
		db.insert(schema.qualificationTypes)
			.values({
				...
				grantsMembershipTier: qt.grantsMembershipTier,
				createdAt: now(),
				updatedAt: now(),
			})
			.run();
...
const qualificationPlan: Array<{
	...
	{
		userIdx: 5,
		typeSlug: "master-school",
		status: "rejected",
		notes: "Could not verify the transcript submitted.",
	},
...
for (const plan of qualificationPlan) {
	const userId =
		plan.userIdx === -1 ? adminUser!.id : testUsers[plan.userIdx]?.id;
	if (!userId) continue;
	const typeId = qualTypeIdBySlug[plan.typeSlug];
	if (!typeId) continue;

	// Upsert qualification
	const exists = existingQuals.find(
		(q) => q.userId === userId && q.typeId === typeId,
	);
	let qualId: string;
	if (!exists) {
		qualId = uuid();
	...
	const existingMembership =
		existingMembershipRows.find((m) => m.userId === userId) ??
		db
			.select()
			.from(schema.userMemberships)
			.all()
			.find((m) => m.userId === userId);
	const currentRank = existingMembership
		? (tierRank[existingMembership.tier] ?? 0)
		: 0;
	...
	const tagExists =
		existingTagRows.find(
			(t) => t.userId === userId && t.slug === plan.typeSlug,
		) ??
		db
			.select()
			.from(schema.userTags)
			.all()
			.find((t) => t.userId === userId && t.slug === plan.typeSlug);
	if (!tagExists) {
...
const counts = {
	...
	participationStatus: db.select().from(schema.participationStatus).all()
		.length,
