...
export const PropertiesPanel = component$<PropertiesPanelProps>((props) => {
	...
	const imageUploadFields =
		props.definition?.configSchema.filter(
			(field) => field.type === "image-upload",
		) ?? [];
					...
					if (field.type === "image-upload") {
						const currentUrl =
							typeof value === "string" && value.length > 0
								? (publicImageUrlFromKey(value, s3BaseUrl.value) ?? value)
								: undefined;
							...
							<div key={field.name}>
								...
								<ImageUploader
									...
									onSettled$={$(() => {
										...
										if (
											settledImageUploaders.value >= imageUploadFields.length
										) {
											void props.onSettled$?.();
										}
									})}
