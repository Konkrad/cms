...
interface ParticipantsTileProps {
  ...
  emptyTitle?: string;
  emptyBody?: string;
  // legacy event props — kept for backwards compat
  eventName?: string;
  // appearance
  variant?: "blue" | "dark";
  area?: string;
  isLoggedIn?: boolean;
  // callback alternative to seeAllHref
  onSeeAll$?: QRL<() => void>;
}
...
export const ParticipantsTile = component$<ParticipantsTileProps>((props) => {
  ...
  // Defaults fall back to event-style copy so existing usages are unaffected
  const contextName = props.eventName ?? "this event";
  ...
  const emptyBody =
    props.emptyBody ?? `Join ${contextName} and invite your friends`;
  ...
  return (
    ...
    >
      ...
      {/* See All — shown for logged-in users with enough participants */}
      {props.isLoggedIn && hasEnough && (
        <>
          {props.seeAllHref ? (
            <a
              href={props.seeAllHref}
              class="flex items-center gap-2 group mt-4 cursor-pointer"
            >
              <SeeAllIcon />
