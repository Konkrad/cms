      ...
      <div class="relative">
        ...
        <div class="relative">
          <input
            ...
            required={required}
            class="w-full px-3 py-2 border border-border-strong rounded-lg focus:ring-2 focus:ring-focus-ring focus:border-transparent"
          ...
          {isLoading.value && (
            <div class="absolute right-3 top-2.5">
              <div class="animate-spin h-5 w-5 border-2 border-gray-300 border-t-primary rounded-full"></div>
            </div>
          )}
        </div>

        {showSuggestions.value && suggestions.value.length > 0 && (
          <div class="absolute z-10 w-full mt-1 bg-white border border-border-strong rounded-lg shadow-lg max-h-60 overflow-y-auto">
            {suggestions.value.map((result, index) => (
              <div
                key={index}
                class="px-4 py-2 hover:bg-primary/5 cursor-pointer border-b border-border last:border-b-0"
