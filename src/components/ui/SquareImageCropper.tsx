...
interface SquareImageCropperProps {
  /** Form field name for the hidden input that holds the uploaded URL */
  name: string;
  label: string;
  /** Currently saved image URL (shown as preview before a new file is selected) */
  currentImageUrl?: string | null;
  /** S3 key prefix, e.g. "public/events/" */
  uploadPath: string;
}

export const SquareImageCropper = component$<SquareImageCropperProps>(
  ({ name, label, currentImageUrl, uploadPath }) => {
    ...
    /** The URL stored in the hidden input (starts as currentImageUrl, updated after upload) */
    ...
    const onPointerMove = $((e: PointerEvent) => {
      ...
      if (dragging.value === "move") {
        ...
        if (newX + newSize > dw) { newSize = dw - newX; }
        if (newY + newSize > dh) { newSize = dh - newY; }
    ...
    const upload = $(async () => {
      ...
      const response = await fetch(
        `/api/images?pipeline=square&filename=${encodeURIComponent(fileId)}`,
        {
          method: "POST",
          body: blob,
          headers: {
        ...
        {!imageDataUrl.value && (
          ...
          >
            ...
            <p class="mt-1 text-xs text-gray-400">
              JPG, PNG or WebP. You'll be able to crop it to a square.
            </p>
          </div>
        )}
                ...
                <>
                  ...
                  >
                    ...
                    <div class="absolute inset-0 pointer-events-none">
                      <div class="absolute bg-white/30" style={{ left: "33.33%", top: "0", width: "1px", height: "100%" }} />
                      <div class="absolute bg-white/30" style={{ left: "66.66%", top: "0", width: "1px", height: "100%" }} />
                      <div class="absolute bg-white/30" style={{ left: "0", top: "33.33%", width: "100%", height: "1px" }} />
                      <div class="absolute bg-white/30" style={{ left: "0", top: "66.66%", width: "100%", height: "1px" }} />
                    </div>
                  </div>
                  ...
                  <div
                    ...
                    style={{ left: `${imageOffsetX.value + crop.x}px`, top: `${crop.y}px` }}
                  ...
                  <div
                    ...
                    style={{ left: `${imageOffsetX.value + crop.x + crop.size}px`, top: `${crop.y}px` }}
                  ...
                  <div
                    ...
                    style={{ left: `${imageOffsetX.value + crop.x}px`, top: `${crop.y + crop.size}px` }}
                  ...
                  <div
                    ...
                    style={{ left: `${imageOffsetX.value + crop.x + crop.size}px`, top: `${crop.y + crop.size}px` }}
