...
- **Tailwind classes**: Use extended palette names (`bg-primary`, `text-accent`, `border-border`, etc.) — generated by the `@theme` block in `theme/tokens/theme.css` (the theme layer owns brand colors).
- **Custom CSS**: Use `var(--color-primary)`, `var(--radius-md)`, etc. (defaults in `src/design/tokens.css`, brand overrides from the theme).
- **Email inline styles**: Import from `~theme/tokens/tokens.ts`.
