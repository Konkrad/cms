...
export const useGroupData = routeLoader$(async (event) => {
	...
	return {
		...group,
		image1Url: publicImageUrlFromKey(group.image1, env.S3_BASE_URL),
		image2Url: publicImageUrlFromKey(group.image2, env.S3_BASE_URL),
		image3Url: publicImageUrlFromKey(group.image3, env.S3_BASE_URL),
	};
});
			...
			<div class="bg-white rounded-lg shadow-sm p-6">
				<form preventdefault:submit onSubmit$={handleSubmit} class="space-y-6">
					...
					<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
						<div>
							...
							{!showImage1Uploader.value && groupData.value.image1 ? (
								<div class="relative rounded-sm border border-gray-200 overflow-hidden">
									...
									<button
										type="button"
										class="absolute bottom-2 right-2 rounded-sm bg-white/90 px-3 py-1.5 text-sm font-medium text-gray-700 shadow-sm hover:bg-white"
										onClick$={() => {
											showImage1Uploader.value = true;
										}}
									>
										Change image
									</button>
						...
						<div>
							<p class="text-sm font-medium text-gray-700 mb-1">
								Image 2 — Middle tile (large)
							</p>
							{!showImage2Uploader.value && groupData.value.image2 ? (
								<div class="relative rounded-sm border border-gray-200 overflow-hidden">
									<img
										src={groupData.value.image2Url || ""}
										alt="Current"
										class="w-full object-cover"
						...
						<div>
							<p class="text-sm font-medium text-gray-700 mb-1">
								Image 3 — Right top tile
							</p>
							{!showImage3Uploader.value && groupData.value.image3 ? (
								<div class="relative rounded-sm border border-gray-200 overflow-hidden">
									<img
										src={groupData.value.image3Url || ""}
										alt="Current"
										class="w-full object-cover"
										style={{ aspectRatio: "1/1" }}
										width={800}
