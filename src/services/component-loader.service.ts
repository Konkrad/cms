...
export const componentLoaderService = {
  ...
  async getComponentDefinition(componentType: string): Promise<BlockDefinition | undefined> {
  ...
  async loadComponent(componentType: string): Promise<any> {
    const loader = componentModules[componentType as keyof typeof componentModules];
    if (!loader) {
