...
test.describe('Tickets', () => {
  ...
  test('paid ticket purchase creates payment intent and processes webhook', async ({ page, db }) => {
    ...
    const stripeClient = new Stripe(process.env.STRIPE_SECRET_KEY || '', { apiVersion: '2025-12-15.clover', host: stripeHost.hostname, port: stripeHost.port ? Number(stripeHost.port) : 12111, protocol: (stripeHost.protocol.replace(':','') as any) });
