...
/** Create a minimal group fixture. Returns the new group's id. */
export function createGroupInDb(opts: {
	name: string;
	slug: string;
	latitude?: string;
	longitude?: string;
	...
	const now = new Date().toISOString();
	db.prepare(
		"INSERT INTO groups (id, name, slug, latitude, longitude, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
	).run(
		id,
		opts.name,
		opts.slug,
...
/** Delete a group fixture (memberships/representatives must be removed separately). */
export function deleteGroupInDb(groupId: string): void {
	const db = openDb();
...
/** Insert a group representative row directly (bypasses the promote action). */
export function addGroupRepresentative(userId: string, groupId: string): void {
	const db = openDb();
	db.prepare(
		"INSERT OR IGNORE INTO group_representatives (id, user_id, group_id, promoted_by, promoted_at) VALUES (?, ?, ?, ?, ?)",
	).run(crypto.randomUUID(), userId, groupId, userId, new Date().toISOString());
	db.close();
}

export function deleteGroupRepresentative(
	userId: string,
	groupId: string,
): void {
	const db = openDb();
...
export function createTestEvent(userId: string): {
	...
	return {
		...
		cleanup() {
			...
			db2
				.prepare("DELETE FROM inventory_groups WHERE id = ?")
...
export function getUserEmailById(userId: string): string | null {
	...
	const user = db
		...
		.get(userId) as { login_id: string | null } | undefined;
	if (!user?.login_id) {
		db.close();
		return null;
	}
	const login = db
		...
		.get(user.login_id) as { email: string } | undefined;
	db.close();
...
export function createTestEventWithProduct(opts: {
	...
	return {
		...
		cleanup() {
			...
			db2
				.prepare(
					"DELETE FROM ticket_participants WHERE ticket_id IN (SELECT id FROM tickets WHERE event_id = ?)",
				)
				.run(eventId);
			...
			db2
				.prepare("DELETE FROM inventory_groups WHERE event_id = ?")
				.run(eventId);
			db2.prepare("DELETE FROM events WHERE id = ?").run(eventId);
			db2.close();
		},
...
export function getJobById(
	...
	const row = db
		.prepare(
			"SELECT id, title, status, suggested_by FROM jobs WHERE id = ? LIMIT 1",
		)
		.get(id) as
		| { id: string; title: string; status: string; suggested_by: string }
		| undefined;
...
/** Create a minimal post fixture. Returns the new post's id. */
export function createPostInDb(opts: {
	title: string;
	body: string;
	userId: string;
	groupId?: string | null;
	...
	db.prepare(
		"INSERT INTO posts (id, title, body, user_id, group_id, visibility, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
	).run(
		id,
		opts.title,
		opts.body,
...
/**
 * Idempotently ensure a published `/news` page with a PostsListBlock exists
 * (page-builder infrastructure the News specs render posts through) — a no-op
 * if it already exists. Never deleted by tests: it's reusable plumbing, not
 * per-test data, so specs only need to manage the posts they create.
 */
export function ensureNewsPage(): { pageId: string } {
	const db = openDb();
	const existing = db
		.prepare(
			"SELECT page_id FROM menu_items WHERE url = '/news' AND page_id IS NOT NULL LIMIT 1",
		)
		.get() as { page_id: string } | undefined;
	if (existing) {
		db.close();
		return { pageId: existing.page_id };
	}
	...
	db.prepare(
		"INSERT INTO pages (id, content, status, created_at, updated_at) VALUES (?, ?, 'published', ?, ?)",
	).run(pageId, content, now, now);
	db.prepare(
		"INSERT INTO menu_items (id, menu_name, title, url, page_id, parent_id, position, status, icon, target, created_at, updated_at) VALUES (?, 'main', 'News', '/news', ?, null, 999, 'hidden', null, '_self', ?, ?)",
	).run(menuItemId, pageId, now, now);
	db.close();
	return { pageId };
}
...
export function getMenuItemByUrl(
	...
	const row = db
		.prepare(
			"SELECT id, title, url, status FROM menu_items WHERE url = ? LIMIT 1",
		)
		.get(url) as
...
export function getMenuItemPosition(id: string): number | null {
	const db = openDb();
	const row = db
		.prepare("SELECT position FROM menu_items WHERE id = ?")
...
type DbHelper = {
	...
	getParticipationStatusByEventIdAndUserId: (
		eventId: string,
		userId: string,
	) => string | null;
...
export const test = base.extend<Fixtures>({
	...
	dbSeed: [
		// biome-ignore lint/correctness/noEmptyPattern: Playwright requires an object-destructuring first param to detect fixture usage
		async ({}, use) => {
	...
	db: async ({}, use) => {
		...
		const helper: DbHelper = {
			...
			getProductsByEventId(eventId) {
				return sqlite
					.prepare("SELECT * FROM products WHERE event_id = ?")
					.all(eventId);
			},
...
export function createQualificationTypeInDb(
	...
	return {
		...
		cleanup() {
			const db2 = openDb();
			db2
				.prepare("DELETE FROM user_qualifications WHERE type_id = ?")
				.run(typeId);
...
export function createMembershipInDb(
	...
	return {
		...
		cleanup() {
			const db2 = openDb();
			db2
				.prepare("DELETE FROM user_memberships WHERE id = ?")
				.run(membershipId);
			db2.close();
		},
	};
...
export function createElectionCycleInDb(
	...
	return {
		...
		cleanup() {
			const db2 = openDb();
			db2
				.prepare("DELETE FROM election_applications WHERE cycle_id = ?")
				.run(cycleId);
			db2
				.prepare("DELETE FROM election_positions WHERE cycle_id = ?")
				.run(cycleId);
...
export function createElectionPositionInDb(
	...
	return {
		...
		cleanup() {
			const db2 = openDb();
			db2
				.prepare("DELETE FROM election_applications WHERE position_id = ?")
				.run(positionId);
			db2
				.prepare("DELETE FROM election_positions WHERE id = ?")
				.run(positionId);
			db2.close();
		},
	};
