				...
				{groups.map((group) => (
					<div key={group.id} class="border rounded-lg p-6 bg-white">
						...
						{group.remainingCapacity === 0 ? (
							<div class="p-4 bg-bg-muted text-text-secondary rounded-sm text-center">
								Sold Out
							</div>
						) : !group.isSalesOpen ? (
							<div
							...
							for (const item of selectedProductDetails) {
								...
								for (let unitNumber = 1; unitNumber <= quantity; unitNumber++) {
									...
									const slots = Array.from(
										...
										(_, slotIdx) => ({
											name:
												isFirstUnitOfProduct && slotIdx === 0 ? buyer.name : "",
											email:
												isFirstUnitOfProduct && slotIdx === 0
													? buyer.email
													: "",
											existingUserId:
												isFirstUnitOfProduct && slotIdx === 0 ? buyer.id : null,
